...
2. `recalcItemCostSheetTotals` — recalcula componentes e totais das fichas de custo afetadas. Componentes do tipo `recipe` usam o custo derivado da composicao atual da receita; componentes do tipo `recipeSheet` usam o total da ficha dependente.
3. `syncMenuItemCostsForItems` — sincroniza uma projeção legada em `MenuItemCostVariation` para cada tamanho do `MenuItem`. Usa a ficha ativa do item como base. Se nao houver ficha por tamanho exato, deriva proporcoes a partir do custo da variacao de referencia (`pizza-medium`).
