...
3. Fichas de custo (`itemCostSheet`) que pertencem aos itens afetados.
4. Fichas de custo com dependencia direta por `itemCostSheetComponent` do tipo `item`, usado para embalagem comprada rastreável.
5. Fichas de custo com dependencia indireta, via `itemCostSheetComponent` do tipo `recipeSheet`.
6. Projecoes legadas de cardapio (`menuItem`) vinculadas a qualquer item do grafo, quando o fluxo ainda depende dessa compat layer.
