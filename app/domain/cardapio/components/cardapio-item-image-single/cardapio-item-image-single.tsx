...
export default function CardapioItemImageSingle({
  ...
  const [videoObjectFit, setVideoObjectFit] = useState<"cover" | "contain">(
    "cover"
  );
  ...
  const inferredKind =
    kind ||
    (/\.(mp4|mov|webm|m4v|ogg|ogv)(\?|$)/i.test(String(src || ""))
      ? "video"
      : "image");
        ...
        isVideo ? (
          <>
            ...
            <video
              ...
              className={cn(
                "absolute inset-0 h-full w-full rounded-[inherit] transition-opacity duration-700 ease-in-out",
