...
const CARDAPIO_INDEX_CACHE_TTL_SECONDS = Number(
  process.env.CARDAPIO_INDEX_CACHE_TTL_SECONDS ?? 60
);
...
export async function loadCardapioIndexData(
  request: Request
): Promise<CardapioIndexLoaderData> {
  const url = new URL(request.url);
  const simulateError = url.searchParams.get("simularErro");
  const simulateErrorByQuery =
    simulateError === "cardapio-index" || simulateError === "cardapio";
  ...
  if (!cachedPayload || simulateErrorByQuery) {
    try {
      ...
      simulateErrorBySetting = parseBooleanSetting(
        simulateErrorSetting?.value,
        false
      );
    } catch (error) {
      console.error(
        "[cardapio._index] non-blocking contingencia.simula.erro load failed, using default",
        error
      );
    }
  }
  ...
  try {
    ...
    await redisSetJson(
      CARDAPIO_INDEX_CACHE_KEY,
      payload,
      Number.isFinite(CARDAPIO_INDEX_CACHE_TTL_SECONDS)
        ? CARDAPIO_INDEX_CACHE_TTL_SECONDS
        : 60
    );
    ...
    if (cachedPayload && isDatabaseConnectivityError(error)) {
      console.warn(
        "[cardapio._index] database unavailable, serving cached payload",
        error
      );
...
async function loadReelUrls(request: Request) {
  ...
  try {
    ...
    console.warn(
      "[cardapio._index] failed loading reels from media API, trying settings fallback",
      error
    );
  }
