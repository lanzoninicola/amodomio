...
- manter snapshots de apoio para revisão e auditoria de preço.
...
- disponibilidade por canal depende de vínculo de canal + preço salvo + flags comerciais.
...
- existe ao menos um `ItemSellingPriceVariation` para canal habilitado

Fórmula prática:

- `canSell && active && channel(cardapio).visible && !upcoming && hasPriceForChannel(cardapio)`

Separação de responsabilidades:

- `ItemSellingChannelItem.visible` responde "o item pode aparecer nesse canal?"
- `ItemSellingPriceVariation.priceAmount` responde "qual preço está salvo para essa variação/canal?"
...
- existência de `ItemSellingPriceVariation` para o canal
...
- diferencie "vínculo no canal", "preço salvo" e "visibilidade pública";
