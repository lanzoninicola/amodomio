  ...
  return db.$transaction(async (tx: any) => {
    ...
    if (options.duplicateCostSheet && source.ItemCostSheet?.length) {
      ...
      if (sourceRoot) {
        ...
        const now = new Date();
        for (const sheet of sourceGroup) {
          const newItemVariationId = variationIdMap.get(sheet.itemVariationId);
          ...
          await tx.itemCostSheet.create({
            data: {
              ...
              status: "active",
              isActive: true,
              baseItemCostSheetId: sheet.id === sourceRootId ? null : newRootId,
