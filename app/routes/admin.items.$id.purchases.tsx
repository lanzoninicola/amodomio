...
export default function AdminItemPurchasesTab() {
  ...
  const latestCost =
    costMetrics?.latestCost ||
    item._itemCostVariationCurrent ||
    item._itemCostVariationHistory?.[0] ||
    null;
  const referenceUnit =
  ...
  const itemUnits: Array<{ id: string; unitCode: string }> =
    item.ItemUnit ?? [];
    ...
    <div className="space-y-4 max-w-7xl">
      {!canConfigureConversion && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          Defina primeiro a unidade de consumo na aba Principal para configurar
          as conversões.
        </div>
      )}
          ...
          <div className="space-y-4">
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              ...
              <span className="font-medium text-slate-600">
                {item.consumptionUm || "não definida"}
              </span>
            </p>
          </div>
          <Button
          ...
          <div className="mt-3 overflow-x-auto rounded-lg border border-slate-100">
            <Table className="min-w-[1080px]">
              <TableHeader className="bg-slate-50/80">
                <TableRow>
                  <TableHead className="h-9 min-w-[120px] px-3 text-xs">
                    Unidade compra
                  </TableHead>
                  <TableHead className="h-9 min-w-[180px] px-3 text-xs">
                    Descrição unidade
                  </TableHead>
                  <TableHead className="h-9 min-w-[110px] px-3 text-xs">
                    Fator
                  </TableHead>
                  <TableHead className="h-9 min-w-[90px] px-3 text-xs">
                    Base
                  </TableHead>
                  <TableHead className="h-9 min-w-[100px] px-3 text-xs">
                    Tipo
                  </TableHead>
                  <TableHead className="h-9 min-w-[360px] px-3 text-xs">
                    Explicação
                  </TableHead>
              ...
              <TableBody>
                {conversions.map((conv) => {
                  ...
                  const restrictedUnit = restrictedUnits.find(
                    (u) => u.code === conv.purchaseUm
                  );
                  return (
                    <TableRow key={conv.id} className="border-slate-100">
                      ...
                      <TableCell className="px-3 py-2 font-mono tabular-nums text-slate-700">
                        {Number(conv.factor).toLocaleString("pt-BR", {
                      ...
                      <TableCell className="px-3 py-2">
                        {restrictedUnit ? (
                          ...
                          <Badge
                            variant="outline"
                            className="border-slate-200 bg-slate-50 text-slate-500 text-xs"
                          >
                            global
                          </Badge>
                        )}
                      ...
                      <TableCell className="px-3 py-2 text-sm text-slate-600">
                        1 {getUnitDisplayName(conv.purchaseUm)} (
                        {conv.purchaseUm}) equivale a{" "}
                      ...
                      <TableCell className="px-3 py-2">
                        <Form method="post" action="..">
                          <input
                            type="hidden"
                            name="_action"
                            value="item-purchase-conversion-delete"
                          />
                          <input
                            type="hidden"
                            name="conversionId"
                          ...
                          <Button
                            ...
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-slate-400 text-red-500"
                          >
                ...
                {pendingLinkedUnits.map((iu) => (
                  ...
                  >
                      ...
                      {canConfigureConversion ? (
                        ...
                        >
                          ...
                          <DecimalInput
                            ...
                            fractionDigits={4}
                            placeholder="0,0000"
                            className="w-28 h-7 text-sm font-mono tabular-nums"
                          />
                          <Button
                            type="submit"
                            size="sm"
                            variant="outline"
                            className="h-7 border-violet-300 text-violet-700 hover:bg-violet-100 text-xs px-2"
                          >
                            Salvar
                          </Button>
                        </Form>
                    ...
                    <TableCell className="px-3 py-2">
                      <Badge
                        variant="outline"
                        className="border-violet-300 bg-violet-100 text-violet-700 text-xs"
                      >
                        sem fator
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="px-3 py-2">
                      <Form method="post" action="..">
                        <input
                          type="hidden"
                          name="_action"
                          value="item-unit-unlink"
                        />
                        <input type="hidden" name="itemUnitId" value={iu.id} />
                        <Button
                          type="submit"
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 text-red-500 hover:bg-red-50 hover:text-red-600"
                          title="Remover vínculo com esta UM"
                          aria-label={`Remover vínculo com ${iu.unitCode}`}
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      </Form>
