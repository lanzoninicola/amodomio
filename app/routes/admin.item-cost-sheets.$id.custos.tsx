...
function calcLineTotal(
  ...
  return Number((baseAmount * wasteFactor).toFixed(6));
}

function formatDifference(value: number) {
  const rounded = Number(value || 0);
  if (Math.abs(rounded) < 0.005) return formatMoney(0);
...
function presetVariationText(
  ...
  const parts = [
    preset.variationLabel,
    preset.variationCode,
    preset.variationKind,
  ].filter(Boolean);
  return parts.join(" · ");
}
...
export default function AdminItemCostSheetCustosTab() {
  ...
  const [expandedIngredientIds, setExpandedIngredientIds] = useState<
    Set<string>
  >(new Set());
  const [selectedRecipeBreakdownTarget, setSelectedRecipeBreakdownTarget] =
    useState<{
      lineId: string;
      itemVariationId: string;
    } | null>(null);
  const defaultManualUnit = unitOptions.includes("UN")
    ? "UN"
    : unitOptions[0] || "";
  const defaultLaborUnit = unitOptions.includes("H") ? "H" : defaultManualUnit;
  const availableReferenceSheets = referenceSheetOptions.filter(
    (sheet) => sheet.id !== rootSheetId
  );
  const isActive = variationSheets.some((sheet: any) => sheet.isActive);
  const manualPresets = componentPresets.filter(
    (preset) => preset.type === "manual"
  );
  const laborPresets = componentPresets.filter(
    (preset) => preset.type === "labor"
  );
  const selectedManualPreset =
    manualPresets.find((preset) => preset.id === selectedManualPresetId) ||
    null;
  const selectedLaborPreset =
    laborPresets.find((preset) => preset.id === selectedLaborPresetId) || null;
  const customPresetOption = {
    value: "__custom__",
    label: "Personalizado",
    searchText: "personalizado livre manual",
  };
  ...
  const referenceSheetSelectOptions = useMemo<SearchableSelectOption[]>(
    () =>
      availableReferenceSheets.map((sheet) => ({
        value: sheet.id,
        label: sheet.name,
        searchText: [sheet.name, Number(sheet.costAmount || 0).toFixed(2)]
          .filter(Boolean)
          .join(" "),
      })),
  ...
  const rowAlerts = useMemo(() => {
    const result: Record<
      string,
      { hasZeroCost: boolean; hasZeroIngredient: boolean }
    > = {};
    for (const line of compositionRows) {
      if (
        ...
        continue;
      const hasZeroCost = line.variationValues.some(
        (v) => Number(v.unitCostAmount || 0) === 0
      );
  ...
  const costCheckVariations = useMemo(
    () =>
      variationSheets.map((sheet: any) => {
        const itemVariationId = String(sheet.itemVariationId || "");
        const lines = compositionRows.map((line) => {
          const value =
            line.variationValues.find(
              (entry) => entry.itemVariationId === itemVariationId
            ) || null;
          const quantity = Number(value?.quantity || 0);
          const wastePerc = Number(value?.wastePerc || 0);
          const unitCostAmount = Number(value?.unitCostAmount || 0);
          const storedTotal = Number(value?.totalCostAmount || 0);
          const expectedTotal = calcLineTotal(
            unitCostAmount,
            quantity,
            wastePerc
          );
          ...
          return {
            ...
            storedTotal,
            expectedTotal,
            difference: storedTotal - expectedTotal,
            breakdown,
          };
        });
        const expectedTotal = lines.reduce(
          (acc, entry) => acc + Number(entry.expectedTotal || 0),
          0
        );
        const storedTotal = Number(totalsByVariationId[itemVariationId] || 0);

        return {
          sheet,
          itemVariationId,
          variationName: variationLabel(sheet),
          storedTotal,
          expectedTotal,
          difference: storedTotal - expectedTotal,
          lines,
  ...
  useEffect(() => {
    function handleSaveShortcut(event: KeyboardEvent) {
      ...
      const activeForm = activeElement?.closest(
        "form[id^='line-form-']"
      ) as HTMLFormElement | null;
      const activeSubmitButton = activeForm?.querySelector<HTMLButtonElement>(
        "[data-cost-sheet-save='true']"
      );
      ...
      const fallbackSubmitButton = document.querySelector<HTMLButtonElement>(
        "[data-cost-sheet-save='true']"
      );
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-800"
            >
            ...
            <div className="max-w-4xl">
              {activeAddForm === "recipe" ? (
                <Form
                  method="post"
                  action={detailPath}
                  className="space-y-3 rounded-[24px] border border-slate-200 bg-slate-50/60 p-5"
                >
                  <input
                    type="hidden"
                ...
                >
                  ...
                  <div>
                    ...
                    <SearchableSelect
                      ...
                      contentClassName="w-[var(--radix-popover-trigger-width)] p-0"
                      renderOption={(option) => {
                        const item = packagingItemOptions.find(
                          (entry) => entry.id === option.value
                        );
                        ...
                        return (
                          <div className="flex min-w-0 flex-col">
                            ...
                            <span className="text-xs text-slate-500">
                              {item.consumptionUm || item.purchaseUm || "UN"} ·{" "}
                              {formatCompactMoney(
                                Number(item.unitCostAmount || 0)
                              )}
                            </span>
                          </div>
                        );
              ...
              {activeAddForm === "sheet" ? (
                ...
                >
                  ...
                  <div>
                    <label
                      className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500"
                      htmlFor="refRecipeSheetId"
                    >
                      Ficha
                    </label>
                    <input
                      type="hidden"
                      name="refRecipeSheetId"
                    ...
                    <SearchableSelect
                      ...
                      contentClassName="w-[var(--radix-popover-trigger-width)] p-0"
                      renderOption={(option) => {
                        const sheet = availableReferenceSheets.find(
                          (entry) => entry.id === option.value
                        );
                        ...
                        return (
                          <div className="flex min-w-0 flex-col">
                            ...
                            <span className="text-xs text-slate-500">
                              R$ {Number(sheet.costAmount || 0).toFixed(2)}
                            </span>
                          </div>
                        );
                      }}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <label
                        className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500"
                        htmlFor="refSheetWastePerc"
                      >
                        Perda %
                      </label>
                      <NumericInput
                        id="refSheetWastePerc"
                        name="wastePerc"
                        min="0"
                        step="0.01"
                        defaultValue="0"
                ...
                >
                  ...
                  {selectedManualPreset ? (
                    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">
                      ...
                      {presetVariationText(selectedManualPreset)}
                    </div>
                  ) : null}
                  <div>
                    <label
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    <div>
                      ...
                      <NumericInput
                        ...
                        step="0.01"
                        defaultValue={Number(
                          selectedManualPreset?.quantity || 1
                        )}
                        decimalScale={2}
                        className="h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                        required
                    ...
                    <div>
                      ...
                      <MoneyInput
                        ...
                        name="unitCostAmount"
                        defaultValue={Number(
                          selectedManualPreset?.unitCostAmount || 0
                        )}
                    ...
                    <div>
                      ...
                      <NumericInput
                        ...
                        step="0.01"
                        defaultValue={Number(
                          selectedManualPreset?.wastePerc || 0
                        )}
                        decimalScale={2}
                        className="h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                      />
                  ...
                  <input
                    key={`manual-notes-${selectedManualPreset?.id || "custom"}`}
                    name="notes"
                    defaultValue={selectedManualPreset?.notes || ""}
                    className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                  ...
                  <div className="flex justify-end">
                    <Button
                      ...
                      variant="outline"
                      name="_action"
                      value="item-cost-sheet-line-add-manual"
                      className="rounded-full border-slate-200 bg-white text-slate-700 hover:bg-slate-100"
                    >
                      Adicionar custo
                    </Button>
              ...
              {activeAddForm === "labor" ? (
                ...
                >
                  ...
                  {selectedLaborPreset ? (
                    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">
                      Aplicacao automatica:{" "}
                      {presetVariationText(selectedLaborPreset)}
                    </div>
                  ) : null}
                  <div>
                    <label
                    ...
                    <input
                      key={`labor-name-${selectedLaborPreset?.id || "custom"}`}
                      id="laborName"
                      name="name"
                      defaultValue={selectedLaborPreset?.name || "Mao de obra"}
                      className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                    />
                  </div>
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    <div>
                      ...
                      <NumericInput
                        ...
                        step="0.01"
                        defaultValue={Number(
                          selectedLaborPreset?.quantity || 1
                        )}
                        decimalScale={2}
                        className="h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                        required
                    ...
                    <div>
                      ...
                      <MoneyInput
                        ...
                        name="unitCostAmount"
                        defaultValue={Number(
                          selectedLaborPreset?.unitCostAmount || 0
                        )}
                        className="h-10 w-full rounded-lg border-slate-200 bg-white px-3 py-2 text-sm"
                        required
                      />
                    ...
                    <div>
                      ...
                      <NumericInput
                        ...
                        step="0.01"
                        defaultValue={Number(
                          selectedLaborPreset?.wastePerc || 0
                        )}
                        decimalScale={2}
                        className="h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                      />
                  ...
                  <input
                    key={`labor-notes-${selectedLaborPreset?.id || "custom"}`}
                    name="notes"
                    defaultValue={selectedLaborPreset?.notes || ""}
                    className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                    placeholder="Observacao opcional"
                  />
                  <div className="flex justify-end">
                    <Button
                      ...
                      variant="outline"
                      name="_action"
                      value="item-cost-sheet-line-add-labor"
                      className="rounded-full border-slate-200 bg-white text-slate-700 hover:bg-slate-100"
                    >
        ...
        <section className="">
          <div className="border-b border-slate-200 px-5 py-4">
            ...
            <div className="mt-1 text-sm text-slate-500">
              Edite cada componente por variacao com a mesma leitura visual da
              grade de precos.
            </div>
          </div>
          ...
          {(() => {
            ...
            for (const line of compositionRows) {
              ...
              const zeroVariations = line.variationValues.filter(
                (v) => Number(v.unitCostAmount || 0) === 0
              );
              if (zeroVariations.length > 0) {
                const zeroVariationIds = new Set(
                  zeroVariations.map((v) => v.itemVariationId)
                );
                const labels = (variationSheets as any[])
                  .filter((sheet) =>
                    zeroVariationIds.has(String(sheet.itemVariationId || ""))
                  )
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead className="bg-white">
                <tr>
                  <th className="sticky left-0 z-20 bg-white px-3 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                    Componente
                  </th>
                  {variationSheets.map((sheet: any) => (
                    <th
                      key={sheet.id}
                      className="min-w-[292px] px-3 py-4 text-left text-xs font-semibold "
                    >
                      <div className="text-[15px] font-semibold text-slate-700">
                        {variationLabel(sheet)}
                      </div>
                  ...
                  <th className="sticky right-0 z-20 bg-white px-3 py-4 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">
                    Acoes
                  </th>
                </tr>
              </thead>
              <tbody>
                {compositionRows.length === 0 ? (
                  <tr>
                    <td
                      colSpan={variationSheets.length + 2}
                  ...
                  compositionRows.map((line) => {
                    const refLocked =
                      ...
                      line.type === "item";
                    const recipeHref =
                      line.type === "recipe" && line.refId
                        ? `/admin/recipes/${line.refId}`
                        : null;
                    const itemHref =
                      line.type === "item" && line.refId
                        ? `/admin/items/${line.refId}/main`
                        : null;
                      ...
                      <tr key={line.id}>
                        ...
                        {variationSheets.map((sheet: any) => {
                          const value =
                            line.variationValues.find(
                              (row) =>
                                row.itemVariationId === sheet.itemVariationId
                            ) || null;
                          const isZeroRefCost =
                            refLocked &&
                            Number(value?.unitCostAmount || 0) === 0;
                          return (
                            <td
                              key={sheet.id}
                              ...
                              >
                                <div className="grid grid-cols-[78px_74px_94px] gap-2">
                                  <div className="space-y-1">
                                    ...
                                    {refLocked ? (
                                      <input
                                        ...
                                        readOnly
                                        className="h-8 w-full rounded-lg border border-slate-100 bg-slate-50 px-2.5 text-xs text-slate-500"
                                      />
                                    ) : (
                                      <Select
                                        name={`unit__${sheet.itemVariationId}`}
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <NumericInput
                                      form={lineFormId}
                                      name={`quantity__${sheet.itemVariationId}`}
                                      min="0.01"
                                      step="0.01"
                                      defaultValue={Number(
                                        value?.quantity || 0
                                      ...
                                      decimalScale={2}
                                      className="h-8 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs text-right"
                                      required
                                    />
                                  </div>
                                  <div className="space-y-1">
                                ...
                                <div className="mt-2 grid grid-cols-[74px_96px] gap-2">
                                  <div className="space-y-1">
                                    ...
                                    <NumericInput
                                      form={lineFormId}
                                      name={`wastePerc__${sheet.itemVariationId}`}
                                      min="0"
                                      step="0.01"
                                      defaultValue={Number(
                                        value?.wastePerc || 0
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    {line.type === "recipe" ? (
                                      ...
                                      >
                                        {formatCompactMoney(
                                          Number(value?.totalCostAmount || 0)
                                        )}
                                      </button>
                                    ) : (
                                      <NumericInput
                                        defaultValue={Number(
                                          value?.totalCostAmount || 0
                                        ).toFixed(2)}
                                        decimalScale={2}
                                        readOnly
                          ...
                          <div className="inline-grid grid-cols-2 gap-3 pt-10">
                            ...
                            >
                              ...
                              <button
                                ...
                                className="flex h-8 w-9 items-center justify-center rounded-full border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50"
                                title="Subir"
                              >
                                ↑
                              </button>
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              ...
              <DialogDescription className="mt-1 text-sm text-slate-500">
                Atualizando custos com base nas receitas e ingredientes.
                Aguarde...
              </DialogDescription>
            </div>
          </div>
      ...
      <Dialog open={costCheckOpen} onOpenChange={setCostCheckOpen}>
        <DialogContent className="max-h-[88vh] max-w-6xl overflow-hidden rounded-2xl p-0">
          <DialogHeader className="border-b border-slate-200 px-6 py-5">
            <DialogTitle className="text-base font-semibold text-slate-950">
              Verificacao do calculo do custo
            </DialogTitle>
            <DialogDescription className="text-sm text-slate-500">
              Conferencia humana da ficha, linha por linha. A conta da linha e:
              custo unitario x quantidade x (1 + perda / 100).
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <div className="mt-1 text-sm font-semibold text-slate-900">
                  {selectedSheet?.name || "Ficha de custo"}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <div className="mt-1 font-mono text-lg font-semibold text-slate-900">
                  {variationSheets.length}
                </div>
              </div>
            </div>

            {costCheckVariations.map((variation) => (
              <section
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">
                      Soma das linhas calculadas e comparada com o total salvo
                      na ficha.
                    </p>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-right">
                    <div>
                      ...
                      <div className="font-mono text-sm font-semibold text-slate-900">
                        {formatMoney(variation.expectedTotal)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                      ...
                      <div className="font-mono text-sm font-semibold text-slate-900">
                        {formatMoney(variation.storedTotal)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full border-separate border-spacing-0 text-sm">
                    <thead>
                      <tr>
                        <th className="border-b border-slate-100 px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                          Componente
                        </th>
                        ...
                        <th className="border-b border-slate-100 px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                          Total salvo
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {variation.lines.length === 0 ? (
                        <tr>
                          ...
                          >
                            Nenhum componente para verificar.
                          </td>
                        </tr>
                      ) : (
                        variation.lines.map((entry) => {
                          const isYieldRecipe =
                            String(entry.breakdown?.recipeCostingMode || "") ===
                            "yield";
                          return (
                            <tr key={entry.line.id}>
                              <td className="border-b border-slate-100 px-4 py-3 align-top">
                                <div className="flex items-center gap-2">
                                  <SheetTypeLabel type={entry.line.type} />
                                  ...
                                  <div className="mt-2 space-y-1 text-xs text-slate-500">
                                    ...
                                    {isYieldRecipe ? (
                                      <div>
                                        Rendimento:{" "}
                                        {formatQuantity(
                                          entry.breakdown.yieldQuantity,
                                          3
                                        )}{" "}
                                        {entry.breakdown.yieldUnit || "UN"};
                                        custo usado por unidade:{" "}
                                        {formatMoney(
                                          entry.breakdown.unitCostAmount
                                        )}
                                      </div>
                                    ) : (
                                      <div>
                                        Custo da receita para esta variacao:{" "}
                                        {formatMoney(
                                          entry.breakdown.unitCostAmount
                                        )}
                                      </div>
                              ...
                              <td className="border-b border-slate-100 px-3 py-3 text-right font-mono text-slate-700">
                                {formatMoney(entry.unitCostAmount)}
                              </td>
                              <td className="border-b border-slate-100 px-3 py-3 text-right font-mono text-slate-700">
                                {formatQuantity(entry.quantity, 3)}
                              </td>
                              <td className="border-b border-slate-100 px-3 py-3 text-right font-mono text-slate-700">
                                {formatQuantity(entry.wastePerc, 2)}%
                              </td>
                              <td className="border-b border-slate-100 px-3 py-3 text-right font-mono font-semibold text-slate-900">
                                {formatMoney(entry.expectedTotal)}
                              </td>
                              <td className="border-b border-slate-100 px-4 py-3 text-right font-mono font-semibold text-slate-900">
                                {formatMoney(entry.storedTotal)}
                              </td>
                            </tr>
                          );
                        })
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) {
            setSelectedRecipeBreakdownTarget(null);
            setExpandedIngredientIds(new Set());
          }
        }}
      >
        <DialogContent className="flex max-h-[85vh] max-w-5xl flex-col overflow-hidden rounded-2xl p-0">
          <DialogHeader className="border-b border-slate-200 px-6 py-5">
            <DialogTitle className="text-base font-semibold text-slate-950">
              Composicao da receita
            </DialogTitle>
            <DialogDescription className="text-sm text-slate-500">
              {selectedRecipeBreakdown?.recipeName ||
                selectedRecipeLine?.name ||
                "Receita"}
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-5">
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <div className="mt-1 font-mono text-lg font-semibold text-slate-900">
                  {formatQuantity(
                    Number(selectedRecipeValue?.quantity || 0),
                    2
                  )}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <div className="mt-1 font-mono text-lg font-semibold text-slate-900">
                  {formatQuantity(
                    Number(selectedRecipeValue?.wastePerc || 0),
                    2
                  )}
                  %
                </div>
              ...
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                ...
                <div className="mt-1 font-mono text-lg font-semibold text-emerald-900">
                  {formatMoney(
                    Number(selectedRecipeValue?.totalCostAmount || 0)
                  )}
                </div>
              </div>
            </div>
              ...
              <table className="min-w-full border-separate border-spacing-0 text-sm">
                <thead className="bg-white">
                  <tr>
                    ...
                    <th className="border-b border-slate-200 px-3 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                      Qtd.
                    </th>
                    <th className="border-b border-slate-200 px-3 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                      Custo un.
                ...
                <tbody>
                  {(selectedRecipeBreakdown?.lines || []).length === 0 ? (
                    <tr>
                      ...
                      >
                        Nenhum ingrediente encontrado para esta variacao da
                        receita.
                      </td>
                    </tr>
                  ) : (
                    (selectedRecipeBreakdown?.lines || []).map((ingredient) => {
                      const isExpanded = expandedIngredientIds.has(
                        ingredient.ingredientId
                      );
                      const hasSubRecipe =
                        Boolean(ingredient.subRecipeLines) &&
                        (ingredient.subRecipeLines?.length ?? 0) > 0;
                      return (
                              ...
                              <div className="flex items-start gap-1.5">
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      setExpandedIngredientIds((prev) => {
                                        ...
                                        else next.add(ingredient.ingredientId);
                                        return next;
                                      })
                                    }
                                    className="mt-0.5 flex-shrink-0 text-emerald-600 hover:text-emerald-800"
                                    title="Ver composicao da sub-receita"
                                  >
                                    {isExpanded ? (
                                ...
                                <div>
                                  ...
                                  <div className="mt-0.5 text-xs text-slate-500">
                                    {[
                                      ingredient.ingredientVariationLabel,
                                      ingredient.unit,
                                    ]
                                      .filter(Boolean)
                                      .join(" · ")}
                                ...
                                {(() => {
                                  ...
                                  const scaleFactor =
                                    isYield && yieldQty > 0
                                      ? usedQty / yieldQty
                                      : 1;
                                  const batchTotal = subLines.reduce(
                                    (acc, l) =>
                                      acc +
                                  ...
                                  const scaledTotal = subLines.reduce(
                                    (acc, l) =>
                                      acc + Number(l.totalCostAmount || 0),
                                    0
                                  );
                                  return (
                                    <div className="ml-5 overflow-hidden  bg-white">
                                      ...
                                      <table className="min-w-full border-separate border-spacing-0 text-xs">
                                        <thead>
                                          <tr>
                                            <th className="border-b border-emerald-100 px-4 py-2 text-left text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                                              Ingrediente
                                            </th>
                                            {isYield ? (
                                            ...
                                            <th className="border-b border-emerald-100 px-4 py-2 text-right text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                                              Total
                                            </th>
                                          </tr>
                                        </thead>
                                        <tbody>
                                          {subLines.map((subLine) => (
                                            <tr key={subLine.ingredientId}>
                                              <td className="border-b border-emerald-50 px-4 py-2 align-top">
                                              ...
                                              {isYield ? (
                                                <td className="border-b border-emerald-50 px-3 py-2 text-right font-mono text-slate-400">
                                                  {formatQuantity(
                                                    subLine.batchQuantity
                                                  )}
                                                </td>
                                              ) : null}
                                              ...
                                              <td className="border-b border-emerald-50 px-3 py-2 text-right font-mono text-slate-600">
                                                {formatMoney(
                                                  subLine.avgUnitCostAmount
                                                )}
                                              </td>
                                              <td className="border-b border-emerald-50 px-4 py-2 text-right font-mono font-semibold text-slate-800">
                                                {formatMoney(
                                                  subLine.totalCostAmount
                                                )}
                                              </td>
                                            </tr>
                                          ))}
