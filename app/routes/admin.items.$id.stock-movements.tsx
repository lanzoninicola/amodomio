...
export default function AdminItemStockMovementsTab() {
  ...
  const from = payload.from
    ...
    : buildWindowStart();
  const fromLabel = Number.isNaN(from.getTime())
    ? `${windowDays} dias`
    : from.toLocaleDateString("pt-BR");
  const isTruncated = Number(pagination.totalItems || 0) > rows.length;
  const currentPath = `/admin/items/${item.id}/stock-movements`;

  return (
    <div className="space-y-4">
      {!item.canStock ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          Este item está com <span className="font-semibold">Tem estoque</span>{" "}
          desativado na aba Principal. Abaixo aparecem apenas movimentações já
          registradas.
        </div>
      ) : null}
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-600">
              Movimentações deste item desde {fromLabel}, incluindo registros
              ativos e eliminados.
            </p>
            <Link
      ...
      <section className="rounded-xl border border-slate-200 bg-white">
            ...
            <TableBody>
              {rows.length === 0 ? (
                <TableRow>
                  ...
                  >
                    Nenhuma movimentação encontrada para este item nos últimos{" "}
                    {windowDays} dias.
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={row.id} className="border-slate-100 align-top">
                    <TableCell className="px-3 py-3 text-xs text-slate-700">
                      ...
                      <div className="text-slate-500">
                        aplicado em {formatDate(row.appliedAt)}
                      </div>
                    ...
                    <TableCell className="px-3 py-3 text-xs text-slate-700">
                      ...
                      <div className="text-slate-400">
                        {row.supplierCnpj || "sem CNPJ"}
                      </div>
                    </TableCell>
                    <TableCell className="px-3 py-3 text-xs text-slate-700">
                      <div className="font-medium text-slate-900">
                        {row.Line?.ingredientName || item.name}
                      </div>
                      <div className="text-slate-500">
                    ...
                    <TableCell className="px-3 py-3 text-xs text-slate-700">
                      <div>
                        entrada: {row.Line?.qtyEntry ?? "-"}{" "}
                        {row.Line?.unitEntry || ""}
                      </div>
                      <div className="text-slate-500">
                        consumo: {row.Line?.qtyConsumption ?? "-"}{" "}
                    ...
                    <TableCell className="px-3 py-3 text-xs text-slate-700">
                      ...
                      <div className="text-slate-500">
                        total doc.: {formatMoney(row.Line?.costTotalAmount)}
                      </div>
                      <div className="text-slate-400">
                    ...
                    <TableCell className="px-3 py-3 text-xs">
                      <Badge
                        variant="outline"
                        className={movementLifecycleBadgeClass(row.deletedAt)}
                      >
                        {row.deletedAt ? "eliminada" : "ativa"}
                      </Badge>
                      {row.deletedAt ? (
                        <div className="mt-1 text-slate-500">
                          eliminada em {formatDate(row.deletedAt)}
                        </div>
                      ) : null}
                    </TableCell>
                    <TableCell className="px-3 py-3 text-xs text-slate-700">
                      {row.id && !row.deletedAt ? (
                        <Link
                          ...
                          className="inline-flex h-8 items-center gap-2 rounded-md border border-slate-200 px-3 text-[11px] font-medium text-slate-700 transition hover:bg-slate-50"
                        >
                          <Pencil size={14} />
                          Editar
                        </Link>
        ...
        {isTruncated ? (
          <div className="border-t border-slate-100 px-4 py-3 text-sm text-slate-500">
            Exibindo as primeiras {rows.length} movimentações do período. Use a
            listagem completa para refinar filtros.
          </div>
        ) : null}
      </section>
    </div>
