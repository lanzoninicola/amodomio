...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (action === "initialize-defaults") {
      ...
      const sizeSortOrderByKey: Record<string, number> = {
        "pizza-medium": 0,
        individual: 1,
        "pizza-small": 2,
        "pizza-bigger": 3,
      ...
      for (const size of menuItemSizes || []) {
        ...
        const sortOrderIndex = sizeSortOrderByKey[code] ?? 0;
    ...
    return json(
      {
        formError:
          error instanceof Error ? error.message : "Não foi possível processar a ação.",
...
export default function AdminVariationsIndexRoute() {
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <Form method="post" className="grid gap-3 md:grid-cols-4">
          ...
          <div>
            ...
            <Input id="sortOrderIndex" name="sortOrderIndex" type="number" defaultValue="0" required />
          </div>
          <div className="md:col-span-4">
            ...
            <Textarea
              ...
              placeholder="Observações, contexto de uso, regra de negócio..."
            ...
            {variations.length === 0 ? (
              ...
              variations.map((variation: any) => (
                <TableRow key={variation.id} className="border-slate-100 hover:bg-slate-50/50">
                  ...
                  <TableCell className="max-w-md px-4 py-3 text-sm text-slate-600">
                    {variation.additionalInformation ? (
                      <span className="line-clamp-3 whitespace-pre-wrap">{variation.additionalInformation}</span>
                    ) : (
                      <span className="text-slate-400">Sem informações adicionais</span>
                    )}
                  </TableCell>
                  <TableCell className="px-4 py-3">
