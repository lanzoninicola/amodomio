...
export default function AdminMobileEntradaEstoqueFotoUnicaPage() {
  ...
  useEffect(() => {
    if (previewFetcher.state === "idle" && previewFetcher.data?.status === 200) {
      ...
      const missingDate = !previewFetcher.data?.payload?.document?.movementAt;
  ...
  const missingDateFromChatGpt = previewPayload && !previewPayload.document?.movementAt;
          ...
          {dateOpen && (
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                <label htmlFor="manualMovementAt" className="text-sm font-medium text-slate-700 whitespace-nowrap w-28 shrink-0">
                  Data movimento
                </label>
                <Input
                  id="manualMovementAt"
