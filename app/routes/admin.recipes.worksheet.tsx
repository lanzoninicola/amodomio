...
import { defer, type ActionFunctionArgs } from "@remix-run/node";
import {
  Await,
  useLoaderData,
  useFetcher,
...
function calcNome(
  recipe: Pick<WorksheetRecipe, "Item" | "Variation">
): string | null {
...
async function loadRecipeWorksheetPayload(): Promise<RecipeWorksheetPayload> {
  ...
  return { recipes, items, unitOptions, variations };
}
...
export async function loader() {
  return defer({
    status: 200,
    message: "Ok",
    payload: loadRecipeWorksheetPayload(),
  });
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (_action === "recipe-update") {
    ...
    if (itemId && variationId) {
      const itemVariation =
        (await db.itemVariation.findFirst({
...
const CELL_EDITABLE = cn(
  CELL,
  "focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-inset focus-within:z-10 relative"
);
const CELL_RECIPE_EDITABLE = cn(
  CELL_RECIPE,
  "focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-inset focus-within:z-10 relative"
);
const INPUT_BASE =
  "w-full h-full px-2 bg-transparent border-0 outline-none text-sm text-slate-700 placeholder:text-slate-300";
...
function limitRecipesByLines(
  ...
  for (const recipe of recipes) {
    ...
    if (lineCount <= remaining) {
      ...
      output.push({
        ...recipe,
        RecipeLine: recipe.RecipeLine.slice(0, remaining),
      });
      remaining = 0;
    }
  }
...
export default function RecipeWorksheet() {
  ...
  return (
    <Suspense fallback={<RecipeWorksheetLoading />}>
      <Await
        resolve={loaderData.payload}
        errorElement={<RecipeWorksheetError />}
      >
        {(payload) => <RecipeWorksheetContent payload={payload} />}
      </Await>
    </Suspense>
  );
}

function RecipeWorksheetContent({
  payload,
}: {
  payload: RecipeWorksheetPayload;
}) {
  const recipes = payload.recipes;
  const items = payload.items;
  const unitOptions = payload.unitOptions ?? UNIT_FALLBACK;
  ...
  const startFlatResize = useCallback(
    (col: number, clientX: number) => {
      resizeRef.current = {
        col,
        startX: clientX,
        startW: flatColWidths[col],
        flat: true,
      };
      setIsResizing(true);
    },
  ...
  const filteredRecipes = recipes.filter((r) => {
    ...
    if (filterVariationId !== "__all__" && r.variationId !== filterVariationId)
      return false;
    return true;
  });
  const totalLines = filteredRecipes.reduce(
    (acc, r) => acc + r.RecipeLine.length,
    0
  );
  const limitedRecipes = limitRecipesByLines(filteredRecipes, visibleLineCount);
  const shownLines = limitedRecipes.reduce(
    (acc, r) => acc + r.RecipeLine.length,
    0
  );
      ...
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm px-3 py-2 flex items-center gap-3 flex-wrap">
        <div className="relative">
          ...
          <Input
            ...
            className="pl-8 h-7 text-xs min-w-[200px]"
            value={search}
        ...
        {view === "grouped" && (
          <>
            ...
            <Button
              ...
              className="h-7 text-xs px-2"
              onClick={() =>
                setCollapsed(new Set(filteredRecipes.map((r) => r.id)))
              }
            >
              Recolher tudo
            </Button>
        ...
        <span className="text-xs text-slate-500 tabular-nums shrink-0">
          ...
          receitas
          {" · "}
          <span className="font-bold text-slate-700">{totalLines}</span>{" "}
          ingredientes
        </span>
        <span className="text-xs text-slate-500 tabular-nums shrink-0">
          Mostrando{" "}
          <span className="font-bold text-slate-700">{shownLines}</span> de{" "}
        ...
        <div className="overflow-x-auto">
          {view === "flat" ? (
            <table
              ...
              style={{
                tableLayout: "fixed",
                width: `${flatColWidths.reduce((a, b) => a + b, 0)}px`,
                minWidth: "100%",
              }}
            >
              <colgroup>
                {flatColWidths.map((w, i) => (
              ...
              <thead>
                <tr>
                  ...
                  <ColHeader colIndex={1} onStartResize={startFlatResize}>
                    Receita
                  </ColHeader>
                  <ColHeader colIndex={2} onStartResize={startFlatResize}>
                    Ingrediente
                  </ColHeader>
                  <ColHeader colIndex={3} onStartResize={startFlatResize}>
                    Variação
                  </ColHeader>
                  <ColHeader colIndex={4} onStartResize={startFlatResize}>
                    UM
                  </ColHeader>
                  <ColHeader
                    colIndex={5}
                    onStartResize={startFlatResize}
                    align="right"
                  >
                    Quantidade
                  </ColHeader>
            ...
            >
              ...
              <thead>
                <tr>
                  <th className={CELL_HDR} />
                  <ColHeader colIndex={1} onStartResize={startResize}>
                    Ingrediente
                  </ColHeader>
                  <ColHeader colIndex={2} onStartResize={startResize}>
                    Variação
                  </ColHeader>
                  <ColHeader colIndex={3} onStartResize={startResize}>
                    UM
                  </ColHeader>
                  <ColHeader
                    colIndex={4}
              ...
              <tbody>
                {filteredRecipes.length === 0 && !isCreating && (
                  <tr>
                    <td
                      colSpan={6}
                      className="border border-slate-200 px-4 py-10 text-center text-sm text-slate-400"
                    >
...
function RecipeWorksheetLoading() {
  return (
    <div className="flex flex-col gap-2">
      <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          <div className="h-7 w-[220px] animate-pulse rounded-md bg-slate-100" />
          <div className="h-7 w-[160px] animate-pulse rounded-md bg-slate-100" />
          <div className="h-7 w-[160px] animate-pulse rounded-md bg-slate-100" />
          <div className="h-7 w-[92px] animate-pulse rounded-md bg-slate-100" />
          <div className="h-7 w-[92px] animate-pulse rounded-md bg-slate-100" />
          <div className="ml-auto h-4 w-[220px] animate-pulse rounded bg-slate-100" />
        </div>
      </div>

      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="grid grid-cols-[36px_1fr_120px_72px_100px_36px] border-b border-slate-200 bg-slate-50">
        ...
        {Array.from({ length: 12 }).map((_, index) => (
          <div
            key={index}
            className="grid grid-cols-[36px_1fr_120px_72px_100px_36px] border-b border-slate-100 last:border-b-0"
          >
...
function RecipeWorksheetError() {
  return (
    <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
      Não foi possível carregar o worksheet de receitas.
    </div>
  );
}
...
function ColHeader({
  ...
  return (
    <th
      className={cn(
        CELL_HDR,
        "px-2 font-medium text-xs text-slate-500 whitespace-nowrap relative",
    ...
    >
      ...
      {colIndex !== undefined && onStartResize && (
        <div
          ...
          onMouseDown={(e) => {
            e.preventDefault();
            onStartResize(colIndex, e.clientX);
          }}
        />
      )}
    </th>
...
function RecipeGroup({
  ...
  return (
    <>
      ...
      {/* ── Ingredient rows ── */}
      {!collapsed &&
        recipe.RecipeLine.map((line, idx) => (
          <RecipeLineRow
            key={line.id}
            line={line}
            recipeId={recipe.id}
...
function RecipeLineRow({
  ...
  const save = useCallback(() => {
    ...
    fetcher.submit(
      {
        _action: "recipe-line-update",
        recipeId,
        lineId: line.id,
        lineUnit: unit,
        lineQuantity: String(currentQty),
      },
      { method: "post" }
    );
  ...
  return (
    ...
    >
      ...
      <td
        className={cn(
          CELL,
          "text-center text-xs text-slate-400 bg-slate-50/30 select-none tabular-nums"
        )}
      >
        {rowNumber}
      </td>
                  ...
                  .map((item) => (
                    <CommandItem
                      ...
                      onSelect={() => {
                        ...
                        itemFetcher.submit(
                          {
                            _action: "recipe-line-item-update",
                            recipeId,
                            lineId: line.id,
                            lineItemId: item.id,
                          },
...
function AddingRow({
  ...
  const [unit, setUnit] = useState(
    unitOptions.find((u) => u === "KG") ?? unitOptions[0] ?? "KG"
  );
  ...
  useEffect(() => {
    if (
      fetcher.state === "idle" &&
      fetcher.data &&
      (fetcher.data as { status?: number }).status === 200
    ) {
      onDone();
    }
  }, [fetcher.state, fetcher.data, onDone]);

  const filteredItems = filterItems(items, itemSearch).slice(0, 60);

  const submit = () => {
    if (!selectedItem || qtyValue <= 0) return;
    fetcher.submit(
      {
        _action: "recipe-line-add",
        recipeId,
        lineItemId: selectedItem.id,
        lineUnit: unit,
        lineQuantity: String(qtyValue),
      },
      { method: "post" }
    );
  };
  ...
  const errorMessage =
    fetcher.data && (fetcher.data as { status?: number }).status !== 200
  ...
  return (
    <tr className={cn("h-8 bg-blue-50/20", isPending && "opacity-60")}>
      <td
        className={cn(
          CELL,
          "text-center text-xs text-slate-400 bg-blue-50/30 select-none tabular-nums"
        )}
      >
        {rowNumber}
      </td>
        ...
        <Popover open={comboOpen} onOpenChange={setComboOpen}>
          <PopoverTrigger asChild>
            <button
              type="button"
              className={cn(
                "w-full h-full px-2 text-left text-sm focus:outline-none",
...
function CreatingRecipeRow({
  ...
  return (
    <tr
      className={cn(
        "h-8 bg-green-50/20 border-t-2 border-green-200",
...
function FlatTableBody({
  ...
  const [addingForRecipeId, setAddingForRecipeId] = useState<string | null>(
    null
  );
  const [isCreating, setIsCreating] = useState(false);
  let globalRowIndex = 0;
          ...
          <Fragment key={recipe.id}>
            ...
            {lines.length === 0 ? (
              <tr
                className={cn(
                  "h-8 bg-slate-50/30",
                  recipeIdx > 0 && "border-t-2 border-slate-200"
                )}
              >
                <td
                  className={cn(
                    CELL,
                    "text-center text-xs text-slate-300 bg-slate-50/30"
                  )}
                >
                  —
                </td>
