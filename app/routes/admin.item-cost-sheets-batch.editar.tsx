...
import {
  ...
  updateComponentsInItemCostSheets,
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    const result = await updateComponentsInItemCostSheets({
      rootSheetIds,
      componentType,
      ...
      currentName: String(formData.get("currentName") || "").trim(),
      newName: String(formData.get("newName") || "").trim(),
    ...
    return ok({
      result,
      message: `${result.changed} ficha(s) editada(s), ${result.skipped} sem componente, ${result.errors} erro(s)`,
    });
  } catch (error) {
...
export default function AdminItemCostSheetsBatchEditPage() {
  ...
  const referenceSheetOptions = useMemo(
    () =>
      options.referenceSheets.map((sheet: BatchSheetOption) => ({
        value: sheet.id,
        label: `${sheet.itemName} · ${sheet.name}`,
  ...
  return (
    <div className="space-y-6">
      <Form method="post" className="space-y-5 border-t border-slate-200 pt-5">
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">Componente atual</label>
            <Select
              value={componentType}
              onValueChange={(value) => {
          ...
          {componentType === "recipe" ? (
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">Receita existente</label>
              <SearchableSelect
                value={recipeId}
          ...
          {componentType === "recipeSheet" ? (
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">Ficha existente</label>
              <SearchableSelect
                value={refSheetId}
          ...
          {componentType === "manual" || componentType === "labor" ? (
            <>
              <div className="space-y-2">
                <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">Preset atual</label>
                <SearchableSelect
                  value={presetId}
              ...
              <div className="space-y-2">
                <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">Ou nome atual</label>
                <input
                  name="currentName"
                  className="h-9 w-[240px] rounded-md border border-slate-200 px-3 text-sm"
                  placeholder="Ex.: embalagem"
                />
              </div>
              <div className="space-y-2">
                <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">Novo nome</label>
                <input
                  name="newName"
                  className="h-9 w-[240px] rounded-md border border-slate-200 px-3 text-sm"
        ...
        <div className="flex items-center gap-3">
          <Button type="submit" disabled={selectedSheetIds.length === 0 || submitting} className="gap-2">
            ...
            {submitting ? "Aplicando..." : `Editar em ${selectedSheetIds.length} ficha(s)`}
          </Button>
          <span className="text-sm text-slate-500">Fichas sem o componente selecionado serão ignoradas.</span>
        </div>
      </Form>
      ...
      {result ? (
        <section className="space-y-3 border-t border-slate-200 pt-5">
          <div className="text-sm text-slate-700">
            {result.changed} editada(s) · {result.skipped} sem componente · {result.errors} erro(s)
          </div>
