...
function calcLucro(priceAmount: number, breakdown: ComputedSellingPriceBreakdown) {
  const custoTotal =
    Number(breakdown.custoFichaTecnica || 0) + Number(breakdown.wasteCost || 0);
  ...
  const taxaCanal = isMarketplace
    ? (priceAmount * Number(breakdown.channel?.taxPerc || 0)) / 100
    : 0;
  const lucroValor = priceAmount - custoComDna - taxaCanal;
  const lucroPerc = priceAmount > 0 ? (lucroValor / priceAmount) * 100 : 0;
  return { lucroValor, lucroPerc };
}
...
function ChannelPriceCell({
  ...
  const { lucroValor, lucroPerc } = calcLucro(priceAmount, channelData.computedBreakdown);
