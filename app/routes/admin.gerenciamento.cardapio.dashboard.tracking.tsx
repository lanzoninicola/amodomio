...
export default function AdminGerenciamentoCardapioTracking() {
  ...
  const interestRateChartItems = interestRateItems.map((item) => {
    const rateTotal =
      ...
      <div className="rounded-lg border border-muted bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            Taxa do mes atual e taxa acumulada (filtro ativo) por sabor.
          </p>
        </div>
        {interestRateChartItems.length === 0 ? (
          ...
          <div className="mt-4">
            ...
            <div className="mt-3 rounded-md border border-muted bg-slate-50">
              <div className="grid h-32 auto-cols-fr grid-flow-col items-end">
                {interestRateChartItems.map((item) => {
                  const heightMonth = Math.max(
                    6,
                    Math.round((item.rateMonth / maxInterestRate) * 100)
                  );
                  const heightTotal = Math.max(
                    6,
                    Math.round((item.rateTotal / maxInterestRate) * 100)
                  );
                  return (
                    <div
                      key={item.id}
                      className="flex h-full flex-col items-center justify-center border-l border-slate-200 px-3 py-2 first:border-l-0"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-semibold text-slate-600">
                          {(item.rateMonth * 100).toFixed(1)}%
                        </span>
                        <span className="text-[10px] font-semibold text-slate-500">
                          {(item.rateTotal * 100).toFixed(1)}%
                        </span>
                      </div>
                      <div className="mt-1 flex h-14 items-end gap-2">
                        <div
