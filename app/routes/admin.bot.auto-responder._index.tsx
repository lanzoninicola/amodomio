...
import {
  findMatchingDeterministicRule,
  renderDeterministicTemplate,
} from "~/domain/ai/deterministic-response.server";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "test") {
    ...
    if (!message) {
      return json(
        { ok: false, error: "Informe uma mensagem para testar." },
        { status: 400 }
      );
    }
    ...
    const matched = findMatchingDeterministicRule(rules, message);
    if (!matched) return json({ ok: true, test: { matched: false, message } });
    const rendered = renderDeterministicTemplate(matched.response, snapshot);
    return json({
      ok: true,
      test: {
        matched: true,
  ...
  if (intent === "toggle") {
    ...
    const current = await prismaClient.botAutoResponseRule.findUnique({
      where: { id },
    });
...
export default function IndexPage() {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Teste de correspondência</CardTitle>
          <p className="text-sm text-muted-foreground">
            Simule uma mensagem sem enviar nada pelo WhatsApp. A primeira regra
            ativa por prioridade será exibida.
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
          <Form method="post" className="flex flex-col gap-2 sm:flex-row">
            ...
            <Input
              name="message"
              placeholder="Ex.: Qual é o endereço?"
              required
            />
          ...
          {actionData && "test" in actionData && actionData.test ? (
            <div className="rounded-md border bg-muted/30 p-3 text-sm">
              {actionData.test.matched ? (
                <div className="space-y-2">
                  <p>
                    <strong>Regra:</strong> {actionData.test.label}
                  </p>
                  <p className="whitespace-pre-wrap">
                    <strong>Resposta:</strong> {actionData.test.response}
                  </p>
                  {actionData.test.missing?.length ? (
                    <p className="text-destructive">
                      Variáveis sem valor: {actionData.test.missing.join(", ")}
                    </p>
                  ) : null}
                </div>
              ) : (
                <p>Nenhuma regra corresponde. A mensagem seguiria para a AI.</p>
              )}
            </div>
          ) : null}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2 text-xs">
          {[
            ...
            <code key={key} className="rounded bg-muted px-2 py-1">
              {"{{"}
              {key}
              {"}}"}
            </code>
      ...
      <Card>
        <CardHeader className="flex-row items-end justify-between gap-3">
          <div>
            <CardTitle>Respostas determinísticas</CardTitle>
            <p className="mt-1 text-sm text-muted-foreground">
              Regras ativas são consultadas antes do modelo de AI. Alterações
              chegam ao worker em até um minuto.
            </p>
          </div>
          <Form method="get" className="flex gap-2 items-center">
            ...
            <Button asChild>
              <Link to="/admin/bot/auto-responder/new">Nova Regra</Link>
            </Button>
          </Form>
        </CardHeader>
        <CardContent>
                ...
                {rules.map((r: any, idx: number) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-xs">
                      {r.activeFrom
                        ? new Date(r.activeFrom).toLocaleString()
                        : "—"}{" "}
                      →{" "}
                      {r.activeTo ? new Date(r.activeTo).toLocaleString() : "—"}
                    </TableCell>
                    <TableCell>
                      <Form method="post">
                    ...
                    <TableCell className="text-right space-x-2">
                      <Button asChild size="sm" variant="outline">
                        <Link to={`/admin/bot/auto-responder/${r.id}/edit`}>
                          Editar
                        </Link>
                      </Button>
