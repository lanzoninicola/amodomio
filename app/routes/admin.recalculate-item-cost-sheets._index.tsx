...
import {
  ...
  type ItemCostSheetBulkCostChange,
  type ItemCostSheetBulkCostTrend,
...
type CostChangeDetailsResponse = {
  message?: string;
...
function CostTrendKpi({ trend }: { trend: ItemCostSheetBulkCostTrend }) {
  const config = {
    increase: {
      label: "Aumentar",
      icon: ArrowUpRight,
...
function CostChangeDetails({
  ...
  const fetcher = useFetcher<CostChangeDetailsResponse>();
  const [expanded, setExpanded] = useState(false);
  const loading = fetcher.state !== "idle";
  const changes = fetcher.data?.costChanges;

  function toggleDetails() {
    const nextExpanded = !expanded;
    setExpanded(nextExpanded);
    if (nextExpanded && !fetcher.data && fetcher.state === "idle") {
      fetcher.load(
        `/admin/recalculate-item-cost-sheets/details?rootSheetId=${encodeURIComponent(
          rootSheetId
        )}`
      );
    }
  ...
  return (
    <div className="mt-1" onClick={(event) => event.stopPropagation()}>
      ...
      >
        ...
        />
        {expanded ? "Ocultar insumos" : `Ver insumos (${count})`}
      </button>
          ...
          {loading && !changes ? (
                ...
                {changes.map((change) => (
                  <tr key={change.itemId}>
                    ...
                    <td className="whitespace-nowrap px-3 py-2 text-right text-slate-500">
                      {currencyFormatter.format(change.previousCostAmount)}
                    </td>
                    <td className="whitespace-nowrap px-3 py-2 text-right font-semibold text-slate-800">
                      {currencyFormatter.format(change.costAmount)}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <p className="px-3 py-4 text-xs text-slate-500">
              {fetcher.data?.message ||
                "Nenhuma alteração de insumo encontrada."}
            </p>
          )}
        </div>
      ) : null}
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const selectedChannel =
    channels.find(
      (channel: any) => String(channel.id) === String(filters.channelId || "")
    ) ||
    channels.find(
      (channel: any) =>
        String(channel.key || "")
  ...
  const visibleItemWhere = filters.channelId
    ? {
        ItemSellingChannelItem: {
          some: {
            itemSellingChannelId: filters.channelId,
            visible: true,
          },
        },
      }
  ...
  const rootSheetOptionsPromise = db.itemCostSheet.findMany({
    where: {
      baseItemCostSheetId: null,
      ...(visibleItemWhere ? { Item: { is: visibleItemWhere } } : {}),
    },
    select: {
      id: true,
      itemId: true,
  ...
  const scanPromise = scanItemCostSheetsForBulkRecalculation({
    rootSheetId:
      filters.filterKind === "sheet" ? filters.rootSheetId : undefined,
  ...
  const payload = Promise.all([scanPromise, rootSheetOptionsPromise]).then(
    ([scan, rawSheets]): LoaderData => ({
      scan,
      filters,
      channels: channels.map((channel: any) => ({
        id: String(channel.id || ""),
        key: String(channel.key || ""),
        name: String(channel.name || channel.key || "Canal"),
      })),
      rootSheetOptions: rawSheets.map((sheet: any) => ({
        id: String(sheet.id || ""),
...
export default function AdminRecalculateItemCostSheetsIndex() {
  ...
  return (
    <Suspense fallback={<RecalculationPageSkeleton />}>
      <Await
        resolve={loaderData.payload}
        errorElement={<RecalculationPageError />}
      >
        {(data) => <RecalculationPageContent data={data} />}
      </Await>
    </Suspense>
  );
}
...
function getErrorReason(error: unknown) {
  ...
  return "O servidor não informou detalhes sobre a falha.";
}

function RecalculationPageError() {
  const error = useAsyncError();
  const revalidator = useRevalidator();
  const reloading = revalidator.state !== "idle";
  ...
  return (
    <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
      ...
      <p className="mt-1 break-words">
        <span className="font-medium">Motivo:</span> {getErrorReason(error)}
      </p>
      <button
        type="button"
        disabled={reloading}
        onClick={() => revalidator.revalidate()}
        className="mt-3 inline-flex h-8 items-center gap-2 rounded-md border border-red-300 bg-white px-3 text-xs font-semibold text-red-800 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
      >
        <RefreshCw
...
function RecalculationPageSkeleton() {
  return (
    <div
    ...
    >
      ...
      <div className="flex gap-4">
        <div className="h-9 min-w-[280px] flex-1 rounded bg-slate-100" />
...
function RecalculationPageContent({ data }: { data: LoaderData }) {
  const recalculateFetcher = useFetcher<RecalculateActionResponse>();
  const submitting = recalculateFetcher.state !== "idle";

  const { scan, filters, rootSheetOptions, channels } = data;
  const filterOptions = useMemo(
    () => buildFilterOptions(rootSheetOptions),
    [rootSheetOptions]
  );
  ...
  const [onlyWithComponents, setOnlyWithComponents] = useState(
    filters.onlyWithComponents
  );
  const [channelId, setChannelId] = useState(filters.channelId || "");
  ...
  const firstVisible =
    scan.totalSheets === 0 ? 0 : (scan.page - 1) * scan.pageSize + 1;
  const lastVisible = Math.min(scan.page * scan.pageSize, scan.totalSheets);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-slate-500">
        ...
        {scan.totalSheets > 0 ? (
          <>
            <span>·</span>
            <span>
              exibindo {firstVisible}–{lastVisible}
            </span>
          </>
        ) : null}
      </div>
      ...
      <Form method="get" className="flex flex-wrap items-center gap-4">
        <input
          type="hidden"
          name="filterKind"
          value={selectedFilter?.kind ?? ""}
        />
        <input
          type="hidden"
          name="rootSheetId"
          value={selectedFilter?.rootSheetId ?? ""}
        />
        <input
          type="hidden"
          name="itemId"
        ...
        <div className="w-full sm:w-56">
          ...
          >
            <SelectTrigger className="h-9 border-slate-300 bg-white text-sm shadow-none">
        ...
        <div className="relative min-w-[280px] flex-1">
          ...
          <SearchableSelect
            ...
            renderOption={(option, isSelected) => {
              const kind = filterSelectOptions.find(
      ...
      {/* Bulk action bar */}
      <div
        className={`flex flex-wrap items-center gap-3 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 transition-all ${
          selectedCount > 0
            ? "opacity-100"
                  ...
                  >
                    ...
                    <TableCell className="max-w-[320px] px-4 py-3 align-top">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <CostChangeDetails
                          rootSheetId={sheet.rootSheetId}
                          count={sheet.costChangeCount}
                        />
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="px-4 py-3">
                      <CostTrendKpi trend={sheet.costChangeTrend} />
                    </TableCell>
                    <TableCell className="px-4 py-3">
                      {sheet.isActive ? (
                        <Badge
      ...
      {scan.totalPages > 1 ? (
        <nav
          aria-label="Paginação das fichas técnicas"
          className="flex items-center justify-between border-t border-slate-100 pt-3"
        >
          <span className="text-xs text-slate-500">
            Página {scan.page} de {scan.totalPages}
          </span>
          <div className="flex items-center gap-2">
            {scan.page > 1 ? (
              <Link
                to={getPageHref(scan.page - 1)}
                prefetch="intent"
              ...
              <span className="inline-flex h-8 items-center gap-1 rounded-md border border-slate-100 px-3 text-xs text-slate-300">
                <ChevronLeft className="h-3.5 w-3.5" />
                Anterior
              </span>
            )}
            {scan.page < scan.totalPages ? (
              <Link
                to={getPageHref(scan.page + 1)}
                prefetch="intent"
                className="inline-flex h-8 items-center gap-1 rounded-md border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 hover:bg-slate-50"
